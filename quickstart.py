...
def main():
    ...
    '''
    cal=requests.get("http://linuxproj.ecs.soton.ac.uk/~sk6g16/json_get.php")
    #print(cal)
    wcalendar=cal.text
    wcalendar="Student Timetable"
    if 0==len(wcalendar):
        send('primary')
    else:
        page_token=None
        ...
        for calendar_list_entry in calendar_list['items']:
            if calendar_list_entry['summary']==wcalendar:
                send(calendar_list_entry['id'])
            else:
